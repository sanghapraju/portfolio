...
/** Aurora container (uses CSS you added in src/index.css) */
function Aurora() {
  return (
    <div className="aurora">
      <div className="aurora-layer aurora-a"></div>
      <div className="aurora-layer aurora-b"></div>
      <div className="aurora-layer aurora-c"></div>
    </div>
  )
}

/** Robust typewriter for hero name (no clipping) */
function useHeroTypewriter() {
  useEffect(() => {
    const el = document.getElementById('hero-name'); if (!el) return;
    const text = 'Shreya Sanghamitra';
...
/** Typewriter for textarea placeholder */
function useMessagePlaceholder() {
  useEffect(() => {
    ...
    const prefersReduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches
    const start = () => {
      if (prefersReduced) { el.setAttribute('placeholder', full); return }
...
export default function App() {
  ...
  return (
    <div className="relative min-h-screen text-white">
      {/* Aurora background (uses classes from src/index.css) */}
      <Aurora />
      ...
      <Section id="home" className="flex items-center">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-10 items-center">
          ...
          {/* Hero media: subtle tilt when active + ta-da lines on first view + parallax */}
          <div
            className={`relative h-64 sm:h-80 md:h-96 overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-sky-500/10 to-slate-900/40 transition-transform duration-700 will-change-transform ${active === 'home' ? 'rotate-[0.8deg]' : 'rotate-0'} ${tadaFired['home'] ? 'tada-lines' : ''}`}
            onMouseMove={parallaxMove}
            onMouseLeave={parallaxLeave}
          >
      ...
      <Section id="about">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="mt-4 text-white/80 leading-relaxed">
            I'm an Interaction Technology graduate student at the University of Twente. My work sits at the intersection of human–computer interaction,
            NLP, and generative AI, focusing on intuitive, adaptive systems aligned with real user needs.
          </p>
        </div>
      </Section>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="bg-slate-900 rounded-2xl p-6 max-w-lg w-full relative">
            ...
            {modal.link && <a href={modal.link} className="text-teal-200 underline" target="_blank" rel="noreferrer">Learn more</a>}
          </div>
        </div>
      )}
      ...
      <Section id="contact">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mt-4 flex gap-3">
            <a href="https://www.linkedin.com/in/shreya-sanghamitra" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl border border-white/15 bg-white/10 hover:bg-white/15 px-4 py-2">
      ...
      <footer className="px-6 md:px-10 lg:px-16 py-10 border-t border-white/10 text-white/60">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <a href="https://www.linkedin.com/in/shreya-sanghamitra" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl border border-white/15 bg-white/10 hover:bg-white/15 px-3 py-2"><FaLinkedin aria-hidden /> <span className="hidden sm:inline">LinkedIn</span></a>
      ...
      {/* inline styles for reveal + ta-da (aurora CSS lives in src/index.css) */}
      <style>{`
        [data-reveal] { opacity: 0; transform: translateY(12px); transition: opacity .6s ease, transform .6s ease; }
        .reveal-in { opacity: 1 !important; transform: translateY(0) !important; }

        /* Ta-da spark lines (first-view) */
        .tada-lines::after {
          content: ""; position: absolute; inset: -10px;
          background: repeating-linear-gradient(90deg, rgba(255,255,255,0.45), rgba(255,255,255,0.45) 2px, transparent 2px, transparent 8px);
          mask-image: radial-gradient(circle at 50% 50%, white 28%, transparent 70%);
